...
$(document).ready(function(){
		...
		$(".btnQRview").click(function(){
			...
			$.ajax({
				type:"POST",
				url :"data_master/tampil_qrcode.php",
				data:{ubahBarang:'',kdBarang:kdBarang},
				//dataType: "json",
				success:function(resp){
					$("#dataTambahUbah").hide();
		...
		//tampilkan form tambah data Customer
		$("#btnCusTambah").click(function(){
			$.ajax({url:'data_master/customer_add.php',
				success:function(html){
					$("#dataTambahUbah").hide();
					$("#dataTambahUbah").html(html)
					$("#dataTambahUbah").fadeIn(1000);
		...
		//tangani operasi simpan data baru Customer
		$(document).on('click',"#btnTambahCustomerSimpan",function () {
			$("#formCustomerTambah").submit(function(e){return false;});
			var kode = $('#kode').val();
			var nama = $('#txtNama').val();
			...
			if(nama.length<=0){$('#txtNama').focus();$('.modal-body').html('<p>Nama Customer Tidak Boleh Kosong<p>');$('#modalUser').modal('show');}
			else if(alamat.length<=0){$('#txtAlamat').focus();$('.modal-body').html('<p>Alamat Customer Tidak Boleh Kosong<p>');$('#modalUser').modal('show');}
			else if(telp.length<=0){$('#txtPhone').focus();$('.modal-body').html('<p>No. Telp Customer Tidak Boleh Kosong<p>');$('#modalUser').modal('show');}
			else if(email.length<=0){$('#txtKontak').focus();$('.modal-body').html('<p>Nama Kontak Customer Tidak Boleh Kosong<p>');$('#modalUser').modal('show');}
			else{
				$.ajax({
					type:"POST",
					url:'data_master/customer_proses.php',
					data:{addCustomer:'',dtaKode:kode,dtaNama:nama,dtaAlamat:alamat,dtaKota:kota,dtaTelp:telp,dtaEmail:email,dtaActive:active},
					dataType: "json",
					success:function(resp){
						$("#alertMsg").html(resp.msg1);
						window.setTimeout(function(){window.location=window.location;}, 2000);
		...
		//tampilkan form ubah data customer
		$(".btnCustomerUbah").click(function(){
			var kode = $(this).data('val');
			
			$.ajax({
				type:"POST",
				url :"data_master/customer_edit.php",
				data:{ubahCustomer:'',kode:kode},
				//dataType: "json",
				success:function(resp){
					$("#dataTambahUbah").hide();
					$("#dataTambahUbah").html(resp);
		...
		$(document).on('click',"#btnUbahCustomerSimpan",function () {
			$("#formCustomerUbah").submit(function(e){return false;});
			var kode = $('#kode').val();
			var nama = $('#txtNama').val();
			var alamat = $('#txtAlamat').val();
			var kota = $('#txtKota').val();
			...
			if(nama.length<=0){$('#txtNama').focus();$('.modal-body').html('<p>Nama customer Tidak Boleh Kosong<p>');$('#modalUser').modal('show');}
				...
				$.ajax({
					type:"POST",
					url:'data_master/customer_proses.php',
					data:{ubahCustomer:'',dtaKode:kode,dtaNama:nama,dtaAlamat:alamat,dtaKota:kota,dtaTelp:telp,dtaEmail:email,dtaActive:active},
					dataType: "json",
					success:function(resp){
						$("#alertMsg").html(resp.msg1);
	...
	$("#btnJualSimpan").click(function(){
		...
		var voucherStatus = $('#voucherStatus').val();
			$.ajax({
					type:"POST",
					url:"kasir/penjualan_proses.php",
					data:{simpanJual:'',noFaktur:noFaktur,voucherStatus:voucherStatus},
					dataType: "json",
					success:function(resp){
						$("#alertMsg").html(resp.msg1);
						window.setTimeout(function(){window.location=window.location;}, 0);
